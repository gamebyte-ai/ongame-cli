...
6. **Emit the phase's headline decision (observability) — BEFORE `state_advance`:**
   ...
   ```
   One emit — the phase's headline retention-curve decision (profile + level count + WHY), not per-level. Core maps `phase.output` → a `generation` nested under the phase span, so the trace shows, for this phase, "what context went in → what curve came out → which profile was chosen and why".
