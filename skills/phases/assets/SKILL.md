...
2. `forge_reference` → `{uploadUrl, token}`, then `reference_upload(gameDir, path, uploadUrl, token)`
   → `assetId`.
...
asset per visible object** (twenty-six soldiers become twenty-six assets rather than three types plus a
